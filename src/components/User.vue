<template>
  <div>
    ...
    <UserDetails  
      ...
      v-on:dialogclose="dialogClosed()"></UserDetails>
    ...
    >
      ...
      <el-table-column  label="Privileges" >
        <template slot-scope="scope">
          ...
          <el-popover
            ...
            width="200"
            trigger="hover">
            <span :key="item" v-for="item in scope.row._source.privileges">&nbsp;<el-tag size="mini">{{item}}</el-tag></span>
      ...
      <el-table-column  label="Filters" > 
        ...
        <template slot-scope="scope">
          <el-popover v-if="scope.row._source.filters?true:false"
            placement="left-start"
            title="Filters"
            width="200"
            trigger="hover"            >
            <span :key="item" v-for="item in scope.row._source.filters">&nbsp;<el-tag size="mini">{{item}}</el-tag></span>
      ...
      <template slot="header" slot-scope="scope">
        ...
        <template slot-scope="scope">
          <el-button size="mini" type="primary" plain icon="el-icon-edit" @click="handleView(scope.$index, scope.row)"></el-button>
          <el-button size="mini" type="danger" plain icon="el-icon-delete" @click="handleDelete(scope.$index, scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
    ...
    ,handleAddUser()
    {
       var newrec = {
        ...
        _source: {          
          ...
          firstname: "",
          lastname: "",
          password:"$pbkdf2-sha256$29000$Yqy1VmotpRTCOAdAaO0dww$/c6x7Fns2XPzyWByRQkb.HF4HxYpLFih7smLE5Uqq/Q",
          privileges: [],
          filters: [],
          language:"en",
          phone:""
