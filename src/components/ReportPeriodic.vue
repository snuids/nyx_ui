    ...
    >
      ...
      <el-table-column label="Days">
        <template slot-scope="scope">          
          ...
          <span v-if="scope.row._source.trigger.type=='daily'" v-for="item in scope.row._source.trigger.days" :key="item">&nbsp;
            <el-tag size="mini" type="info">{{(item+1)}}</el-tag>
          </span>
          <span v-if="scope.row._source.trigger.type=='monthly'" v-for="item in scope.row._source.trigger.days" :key="item">&nbsp;
...
export default {
  ...
  methods: {
    ...
    duplicate() {      
      ...
      this.curScheduler._id = "id_" + Math.floor((1 + Math.random()) * 0x1000000);
    ...
    askDeleteScheduler(index, row) {
      this.$confirm('This will permanently delete the scheduler. Continue?', 'Warning', {
