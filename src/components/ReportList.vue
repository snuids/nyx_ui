...
<!--:default-sort="{prop: '_id', order: 'descending'}"-->
    ...
    >
      ...
      <el-table-column label="Privileges" v-if="$store.getters.creds.hasPrivilege('reporteditor')" width="120">
        <template slot-scope="scope">
          <el-popover
      ...
      <el-table-column label="Parameters" v-if="$store.getters.creds.hasPrivilege('reporteditor')" width="120">
        <template slot-scope="scope">
          <el-popover 
      ...
      <el-table-column align="right" width="300">
        ...
        <template slot="header" slot-scope="scope">
          <div>
            ...
            <el-tooltip v-if="$store.getters.creds.hasPrivilege('reporteditor')" class="item" effect="light" content="Add" placement="bottom">
              <el-button
                circle
                size="mini"
                @click="addReport()"
    ...
    <!--el-row v-for="item in tableData" :key="item._id">
    <el-card   class="box-card">
        <div slot="header" class="clearfix">
            <span>{{item._source.title}}</span>
            <el-button size="mini" style="float: right; padding: 3px" type="primary">Generate</el-button>
        </div>
        <div class="text item">
...
export default {
  ...
  methods: {
    ...
    addReport() {
      this.curReportId = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 5);
      this.curReportIndex = 'nyx_reportdef'
