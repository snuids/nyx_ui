<template>
    ...
    <el-tabs v-model="activeName" @tab-click="refresh">
          ...
          <el-row v-for="(line,index) in fields" :key="index">
            <el-col :span="24/modulo" v-for="(field,index2) in line" :key="index2">
              ...
              <el-form-item :label="field.key" :label-width="formLabelWidth">
                <el-tooltip class="item" effect="dark" :content="field.value" placement="right" v-if="typeof field.value == 'string'">
                  <el-input size="mini" v-model="field.value" autocomplete="off" @blur="valueChanged(field)"></el-input>
                </el-tooltip>
                <el-input-number v-if="typeof field.value == 'number'" @change="valueChanged(field)"
                size="mini"
                v-model="field.value"
                autocomplete="off"
      ...
      <el-tab-pane :label="$t('generictable.original')" name="Second" v-if="$store.getters.creds.hasPrivilege(config.config.writeprivileges)">
        <editor v-model="strOrgRec"  
                id="test"
                @init="editorInitRO" 
                lang="yaml" 
                theme="chrome" 
      ...
      <el-tab-pane :label="$t('generictable.new')" name="fifth" v-if="$store.getters.creds.hasPrivilege(config.config.writeprivileges)">
        <editor v-model="strNewRec"  
                id="test"
                @init="editorInit" 
                lang="yaml" 
                theme="chrome" 
